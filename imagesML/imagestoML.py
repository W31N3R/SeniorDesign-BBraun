...
filelist = []
similarity_array = []
foldername = r"C:\Users\herop\OneDrive\Documents\!College\!BBraunFiles\imagesML\22_Gauge_XYZ_Clockwise_Rotations"
image1 = r"C:\Users\herop\OneDrive\Documents\!College\!BBraunFiles\imagesML\22_Gauge_XYZ_Clockwise_Rotations\irlImage2.png"
#image2 = r"C:\Users\herop\OneDrive\Documents\!College\!BBraunFiles\imagesML\22_Gauge_XYZ_Clockwise_Rotations\22_Gauge_XYZ_Clockwise_Rotations-0044.png"
...
def load_images_in_array(path):
    ...
    os.chdir(currentdir) #change back to where it was
    print(os.getcwd())  # Print the current working directory
...
load_images_in_array(foldername)

model = train_siamese_model(foldername)
print("Model trained successfully.")

for file in filelist:
    print("")
    file = f"{foldername}\\{file}"
    print("Predicting similarity score for the following images:")
    print(image1)
    print(file)
    score = predict_similarity(model, image1, file)
    print(f"Similarity Score: {score:.2f}") #the :.2f formats the decimal to a floating point number with two decimal places.
    similarity_array.append(score)
    
highest_similarity = max(similarity_array)
highest_similarity_index = similarity_array.index(highest_similarity)
lowest_similarity = min(similarity_array)
lowest_similarity_index = similarity_array.index(lowest_similarity)

print(f"The most similar image was {filelist[highest_similarity_index]} with a similarity score of {highest_similarity:.2f}")
print(f"The least similar image was {filelist[lowest_similarity_index]} with a similarity score of {lowest_similarity:.2f}")
print(f"The average similarity score was {sum(similarity_array)/len(similarity_array):.2f}")
print(f"The similarity between base and base was {similarity_array[0]:.2f}")